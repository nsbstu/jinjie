...
/*
    ...
    构造方法：
        OutputStreamWriter(OutputStream out, String charsetName)创建使用指定字符集的 OutputStreamWriter。不指定默认使用UTF-8
     ...
     使用步骤：
        ...
        2：创建OutputStreamWriter对象的方法write，把字符转换为字节存储缓冲区中（编码）
        3：创建OutputStreamWriter对象的方法flush，把内存缓存区中的字节刷新到文件中（使用字节流写字节的过程）
        4：释放资源
...
public class Demo02OutputStreamWriter {
    ...
    private static void write_gbk() throws IOException {
        ...
        //OutputStreamWriter osw = new OutputStreamWriter(new FileOutputStream("day10\\utf-8.txt"));
        //2：创建OutputStreamWriter对象的方法write，把字符转换为字节存储缓冲区中（编码）
        osw.write("你好");
        //3：创建OutputStreamWriter对象的方法flush，把内存缓存区中的字节刷新到文件中（使用字节流写字节的过程）
        osw.flush();
